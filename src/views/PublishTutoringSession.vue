			...
			<form>
				...
				<div class="field">
					...
					<div v-if="this.$v.StartOfferTime.$dirty || !validated">
						<span v-if="!$v.StartOfferTime.required"
							>Este campo es obligatorio</span
						>
					</div>
				</div>
				...
				<div class="field">
					...
					<div v-if="this.$v.EndOfferTime.$dirty || !validated">
						<span v-if="!$v.EndOfferTime.required"
							>Este campo es obligatorio</span
						>
					</div>
				</div>
				...
				<div class="field">
					...
					<div
						...
						"
					>
						<span v-if="!$v.TutoringSession.Price.required"
							>Este campo es obligatorio</span
						>
						<span v-if="!$v.TutoringSession.Price.checkPrice"
							>Ingrese un precio valido</span
						>
					</div>
				</div>
...
Este campo es obligatorio</textarea
			...
			<div v-if="TopicsSelected.length > 0">
				<div v-for="topic in TopicsSelected" :key="topic.id">
					<ElementList
						v-bind:element="topic.name"
						v-on:del-element="deleteTopic(topic.id)"
					></ElementList>
				</div>
			</div>
			...
			<div v-if="SessionsCreated.length > 0">
				...
				<div
					class="sessionscreated"
					v-for="(e, index) in SessionsCreated"
					:key="index"
				>
					<p>
						Session N°:
						{{ index }}
						<button v-on:click="updateSession(index)" class="upd">
							x
						</button>
						<button
							v-on:click="deleteSessionAux(index)"
							class="del"
						>
							x
						</button>
			...
			<div class="field" style="margin-top:5%;">
				...
				<div>
					<button
						v-on:click="cancel"
						class="btn"
						style="width: 335px; margin-top:0%"
					>
						Cancelar
					</button>
				</div>
		...
		updateSession(index) {
			...
			for (let i = 0; i < this.TopicsByCourse.length; ++i) {
				for (let j = 0; j < this.TutoringSession.Topics.length; ++j) {
					if (
						this.TopicsByCourse[i].value ===
						this.TutoringSession.Topics[j]
					) {
						this.TopicsSelected.push({
