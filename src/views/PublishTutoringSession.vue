    ...
    <div class="form-container">
      ...
      <form>
        <div class="field">
          ...
          <span
            v-if="
							!$v.TutoringSession.Place.required &&
        ...
        <div class="field">
          ...
          <span
            v-if="
							!$v.SessionDate.required &&
								(this.$v.SessionDate.$dirty || !validated)
						"
          >Este campo es obligatorio</span>
        </div>
        ...
        <div class="field">
          ...
          <input
			...
			v-on:focusout="checkTimes"
            name="HoraInicio"
            placeholder="Ingrese la fecha"
          />

          <div v-if="this.$v.StartOfferTime.$dirty || !validated">
			<span v-if="timeError">La hora de inicio no concuerda con la hora de fin</span>
        ...
        <div class="field">
          ...
          <div v-if="this.$v.EndOfferTime.$dirty || !validated">
			<span v-if="timeError">La hora de fin no concuerda con la hora de inicio</span>
        ...
        <div class="field">
          ...
          <div v-if="
							this.$v.TutoringSession.Price.$dirty || !validated
						">
...
Este campo es obligatorio</textarea>
          <span
            v-if="
							!$v.TutoringSession.Description.required &&
        ...
        <div class="field">
          ...
          <span
            v-if="
							!$v.TopicsSelected.required &&
								(this.$v.TopicsSelected.$dirty || !validated)
						"
          >Este campo es obligatorio</span>
        </div>
      </form>
      ...
      <div v-if="SessionsCreated.length > 0">
        ...
        <div class="sessionscreated" v-for="(e, index) in SessionsCreated" :key="index">
          <p>
            ...
            {{ index }}
            <button
              v-on:click="updateSession(index)"
              class="upd"
            >Modificar</button>
            <button v-on:click="deleteSessionAux(index)" class="del">Eliminar</button>
          </p>
        </div>
      </div>
...
const checkTimesAux = (start, end)  => {

  let startAux = start.split(":");
  let endAux = end.split(":");

  console.log(startAux[0]+startAux[1] +' '+endAux[0]+endAux[1] );
...
export default Vue.extend({
  ...
  methods: {
	...
	checkTimes(){
		if(this.StartOfferTime.length == 5 && this.EndOfferTime.length == 5)
			this.timeError = checkTimesAux(this.StartOfferTime,this.EndOfferTime);
	},
    ...
    checkForm: function() {
      if (
        (this.$v.TutoringSession.Place.$error ||
          ...
          this.$v.TopicsSelected.$error || this.timeError ||
          (!this.$v.TutoringSession.Place.$dirty ||
    ...
    async publish() {
      if (this.SessionsCreated.length > 0) {
        ...
        await offerService.createTutoringOffer(tutoringOfferObj);
