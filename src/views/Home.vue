		...
		<div class="home-sub">
			...
			<div class="tutoringSessionsToAssist">
				...
				<div class="list-sessions-active" v-if="tutoringSessions">
					<div
						class="card-sessionHome"
						v-for="(session, index) in tutoringSessions"
						:key="index"
					>
...
export default Vue.extend({
	...
	data() {
		return {
			courseName: "",
			name: AuthenticationService.userValue.name,
			tutoringSessions: []
		};
	},
	created() {
	...
	methods: {
		...mapActions("HomeModule", ["fetchTutoringOffers", "fetchTutors"]),
		init: async function() {
		...
		findTutoringOffersAndTutors: async function() {
			if (this.courseName.length === 0) {
				return;
			}

			try {
				await this.fetchTutoringOffers(this.courseName.toLowerCase());
				await this.fetchTutors(this.courseName.toLowerCase());
			} catch (error) {
				console.log("ERROR EN HOME => FIND_TUTORING_OFFERS_AND_TUTORS");
			}
		}
	}
});
