<template>
	<form
		class="add-course-modal-component-container"
		v-on:submit.prevent="addCourse"
	>
		<div class="image" v-bind:style="{ backgroundColor: backgroundColor }">
		...
		<div class="add-course-data">
			...
			<p class="error-message" v-if="errorType === 1">
				Error de aplicación
			</p>
			<p class="error-message" v-if="errorType === 2">
				Curso no encontrado
			</p>
			<p class="error-message" v-if="errorType === 3">
				Tutor no encontrado
			</p>
	...
	methods: {
		...
		addCourse: async function() {
			if (this.courseName !== "") {
				try {
					this.errorType = 0;
					const userId: number = AuthenticationService.userValue.id;
